...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <!-- Modal Window -->
    <div class="relative w-full max-w-3xl glass-panel p-6 border border-dark-700 shadow-2xl space-y-6 max-h-[90vh] flex flex-col">
      <!-- Header -->
      <div class="flex items-center justify-between border-b border-dark-700/60 pb-4 flex-shrink-0">
        <div class="flex items-center space-x-3">
          <div class="h-8 w-8 rounded-xl bg-brand-600 flex items-center justify-center font-bold text-white text-xs">
            👁
          </div>
          <div>
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div v-if="isLoading" class="p-12 text-center space-y-6">
          <div class="inline-flex items-center justify-center h-16 w-16 rounded-full bg-brand-600/20 text-brand-400 border border-brand-500/30 animate-pulse">
            ✨
          </div>
          <div class="space-y-2">
      ...
      <div class="pt-4 border-t border-dark-700/60 flex items-center justify-between flex-shrink-0">
        <button
          @click="fetchPreview(true)"
          :disabled="isLoading"
          class="px-4 py-2 rounded-xl bg-brand-600/20 hover:bg-brand-600/30 text-brand-400 hover:text-brand-300 border border-brand-500/30 text-xs font-semibold flex items-center gap-2 transition-all disabled:opacity-50"
        >
        ...
        <button
          @click="emit('close')"
          class="px-5 py-2 rounded-xl bg-dark-800 hover:bg-dark-700 text-slate-300 text-xs font-semibold border border-dark-600"
        >
          Fermer
        </button>
      </div>
